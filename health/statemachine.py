...
from utils import write_to_output, get_csv_writer, write_to_csv, get_terminal_decoded_string
import utils
import win32process
import wmi
...
class _Statemachine(object):
	...
	def _list_sockets_network(self):
		for pid in win32process.EnumProcesses():
			try:			
				...
				map_proc= p.as_dict(attrs=['pid', 'name', 'username'])
				local_addr=''
				local_port=''
				remote_addr=''
				remote_port=''
				for connection in p.get_connections():
					...
					yield pid,map_proc['name'],local_addr,local_port,remote_addr,remote_port,connection.status
	...
	def _csv_list_running_process(self,list_running):
		with open(self.output_dir+'_processes.csv','ab') as fw:
			...
			#'"Computer Name"|"Type"|"PID"|"Name"|"Command"|"Path Exec"\r\n'
	...
	def _csv_list_drives(self,drives):
		with open(self.output_dir+'_list_drives.csv','wb') as fw:
			csv_writer = get_csv_writer(fw)
			#'"Computer Name"|"Type"|"Fab"|"Partions"|"Disk"|"File System"\r\n'
	...
	def _csv_list_network_drives(self,drives):
		with open(self.output_dir+'_list_networks_drives.csv','wb') as fw:
			csv_writer = get_csv_writer(fw)
			#'"Computer Name"|"Type"|"disque"|"fs"|"Partition Name"\r\n'
	...
	def _csv_list_sessions(self,sessions):
		with open(self.output_dir+'_sessions.csv','ab') as fw:
			csv_writer = get_csv_writer(fw)
			#'"Computer Name"|"Type"|"Logon ID"|"Authentication Package"|"Start Time"|"Logon Type"\r\n'
	...
	def _csv_list_network_adapters(self,ncs):
		with open(self.output_dir + "_networks_cards.csv",'wb') as fw:
			csv_writer = get_csv_writer(fw)
			#'"Computer Name"|"Type"|"netcard"|"adapter_type"|"description"|"mac_address"|"product_name"|"physical_adapter"|"speed"|"IPv4"|"IPv6"|"DHCP_server"|"DNS_server"|"database_path"|"nbtstat_value"\r\n'
