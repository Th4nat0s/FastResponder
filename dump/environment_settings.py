...
class EnvironmentVariable(object) :
    ...
    def environementVariable(self, currentMachine): 
        listEnvVar = []        
        for ve in currentMachine.Win32_environment():
            envVar = EnvironmentVariable()
            envVar.name = ve.name
            envVar.value = ve.VariableValue
            listEnvVar.append(envVar)
        return listEnvVar
...
class OperatingSystem(object) :
    ...
    def os_informations(self, currentMachine):
        
        for data in currentMachine.Win32_OperatingSystem():
...
class Disks(object) :
        ...
        def getDiskInformations(self, currentMachine):
        
            disks = []
            for physicalDisk in currentMachine.Win32_DiskDrive ():
                        disk = Disks()
...
class Partitions :
    ...
    def partitionInformations(self,currentMachine):
        ...
        for physicalDisk in currentMachine.Win32_DiskDrive ():
            for partitions in physicalDisk.associators("Win32_DiskDriveToDiskPartition"):
                for logicalDisk in partitions.associators("Win32_LogicalDiskToPartition"):
                    ...
                    partition = self.bootSectorInfos(ntfsdrive,partition.type, partition)
                    
                    listPartitions.append(partition)
        return listPartitions 
    
    
  
    def bootSectorInfos(self, part,type, partition):
        
        bootSector = self.saveBootSector(part)
        self.extractHexa(bootSector)
        try :
    ...
    def saveBootSector(self, image):
    
        try :
            fileImage = open(image, "rb")
            fileBoot = open(self.path+os.path.sep+"boot sector", "w")
            fileBoot.write(hexdump.hexdump(fileImage.read(512),"return"))
            fileImage.close()
            fileBoot.close()
        except Exception as inst :
            self.logger.error("Extracting mbr failed")
        return fileBoot.name
    
    
    
    def extractHexa(self,bootSector):
    
        try : 
            file = open(bootSector, "rb")
            hex = ""
            for line in file.readlines():
                hex = hex + line[10:58]
            for i in hex.split(" "):
                self.hexaBootSector = self.hexaBootSector + i 
            file.close()
        except Exception as inst :
    ...
    def __str__(self):
        ...
        if self.type == "NTFS" :
            ...
            output += "\nLogical cluster number of MFT : " + str(self.mftLogicalClusterNumber) + "\nLogical cluster number of MFTMIRR : " + str(self.mftmirrLogicalClusterNumber)
            output += "\nClusters per file record segment : " + str(self.clusterPerFileRecordSegment) + "\nVolume serial number : " + str(self.volumeSerialNumber) +"\n"
        
        elif self.type == "Fat32" :
            output += "\nDisk :"+ self.disk + "\nPartition name :" + self.partName + "\nType :"+ self.type
            output += "\nBlock size :" + str(self.blockSize) + "\nSize :" + str(self.size) + "\nBytes per sector : " + str(self.bytesPerSector)
            output += "\nSectors per cluster : " + str(self.sectorPerCluster) + "\nNumber of heads : " + str(self.numberOfHeads) 
            output += "\nSectors per track : " + str(self.sectorsPerTrack) + "\nMaximum root entry directories :" + str(self.maximumRootEntryDirectories)
            output += "\nNumber of sectors in partition smaller than 32 mb : " + str(self.numberOfSectorsSmaller32mb) 
            output += "\nNumber of sectors per FAT : " + str(self.sectorsNumberFat)+ "\nVersion of FAT32 drive : " + str(self.fat32DriveVersion)
            output += "\nSector number of the FileSystem information sector : " + str(self.sectorNumberFsInformation)
            output += "\nSector number of the backupboot sector : " + str(self.sectorNumberBackupBoot) + "\nSerial number of partition : " + str(self.serialNumberPartition) + "\n"
            ...
            output += "\nBlock size :" + str(self.blockSize) + "\nSize :" + str(self.size) + "\nBytes per sector : " + str(self.bytesPerSector)
            output += "\nSectors per cluster : " + str(self.sectorPerCluster) + "\nNumber of heads : " + str(self.numberOfHeads) 
            output += "\nSectors per track : " + str(self.sectorsPerTrack) + "\nPossible root entry number :" + str(self.possibleRootEntryNumber)
            output += "\nNumber of sectors in partition smaller than 32 mb : " + str(self.numberOfSectorsSmaller32mb)  + "\nFat name : " + str(self.fatName) +"\n"
